  ...
  <div class="row">
    <div class="col-lg-6">
      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          @if (Cache::get('update'))
            <p id='solder-update-ajax' class="alert alert-danger">Solder is out of date. Please refer to the wiki on how
              to update.</p>
          @else
          ...
          <a href="https://docs.solder.io/docs/updating-solder" target="blank_">
            <button id='solder-wiki' class="btn btn-default">Updating Solder <i class="fa fa-question"></i></button>
          </a>
            ...
            @else
              @foreach ($changelog as $change)
                <a href="{{ $change['html_url'] }}" target="blank_" class="list-group-item">
                  <span class="badge"
                        style="margin-left:5px;">{{ date_format(date_create($change['commit']['author']['date']), 'M, d, Y | g:i a') }}</span>
                  <img src="{{ $change['author']['avatar_url'] ?? $change['committer']['avatar_url'] }}"
                       ...
                       height="23" width="23"> {{ $change['commit']['message'] }}
                </a>
              @endforeach
            @endif
          </div>
          <div class="text-right">
            <a href="https://github.com/TechnicPack/TechnicSolder/commits/master">View All Activity <i
