    ...
    <div class="panel-body">
      ...
      <div class="tab-content">
        ...
        <div class="tab-pane fade in active" id="versions">
          <br>
          <p>Solder currently does not support uploading files directly to it. Your repository still needs to exist and
            follow a strict directory structure. When you add versions the URL will be verified to make sure the file
            exists before it is added to Solder. The directory structure for this mod is as follows:</p>
          ...
          <table class="table">
            ...
            <tr id="add-row">
              <form method="post" id="add" action="{{ URL::to('mod/add-version') }}">
                ...
                <td>N/A</td>
                <td>
                  <button type="submit" class="btn btn-success btn-small add">Add Version</button>
                </td>
              </form>
            </tr>
            @foreach ($mod->versions->sortByDesc('id') as $ver)
              <tr class="version" rel="{{ $ver->id }}">
                <form class="rehash-form" data-version-id="{{ $ver->id }}">
                  ...
                  <td><input type="text" class="md5 form-control" name="md5" rel="{{ $ver->id }}"
                             placeholder="{{ $ver->md5 }}"></td>
                  <td class="url" rel="{{ $ver->id }}"><small><a
                        href="{{ config('solder.mirror_url').'mods/'.$mod->name.'/'.$mod->name.'-'.$ver->version.'.zip' }}">{{ config('solder.mirror_url').'mods/'.$mod->name.'/'.$mod->name.'-'.$ver->version.'.zip' }}</a></small>
                  </td>
                  ...
                  @else
                    ...
                    <ul>
                      @foreach ($ver->builds as $build)
                        <li><a href="{{'/modpack/view/'.$build->modpack->id}}">{{$build->modpack->name}}</a> - <a
                            href="{{'/modpack/build/'.$build->id}}">{{$build->version}}</a></li>
                      @endforeach
                    </ul>
                  @endif
                </td>
