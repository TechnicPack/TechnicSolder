...
<nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
  <div class="navbar-header">
    ...
    <a class="navbar-brand" href="{{ URL::to('dashboard') }}"><img
        src="{{ URL::asset('img/title.png') }}"> {{ SOLDER_VERSION }}</a>
  </div>
...
<nav class="navbar-default navbar-static-side" role="navigation">
  <div class="sidebar-collapse">
    <ul class="nav side-menu" id="side-menu">
      <li>
        <a href="{{ URL::to('dashboard') }}" {{ Request::is('dashboard') ? ' class="mm-active"' : '' }}><i
      ...
      <li>
        ...
        <ul class="nav nav-second-level">
          ...
          @foreach ($allModpacks as $modpack)
            @if ($modpack->icon)
              <li><a href="{{ URL::to('modpack/view/'.$modpack->id) }}"><img
                    src="{{ URL::asset('resources/' . $modpack->slug . '/icon.png') }}"
                    style="width: 16px; height: 16px;"> {{ $modpack->name }}{{ $modpack->hidden ? " (Hidden)" : "" }}
                </a></li>
            @else
              <li><a href="{{ URL::to('modpack/view/'.$modpack->id) }}"><img
                    src="{{ URL::asset('resources/default/icon.png') }}"
                    style="width: 16px; height: 16px;"> {{ $modpack->name }}{{ $modpack->hidden ? " (Hidden)" : "" }}
                </a></li>
            @endif
          @endforeach
