		...
		<table class="table table-striped table-bordered table-hover" id="dataTables">
			...
			@foreach ($modpack->builds->sortByDesc('id') as $build)
				<tr>
					...
					<td><input autocomplete="off" type="radio" name="recommended" value="{{ $build->version }}"{{ $modpack->recommended === $build->version ? " checked" : "" }}></td>
					<td><input autocomplete="off" type="radio" name="latest" value="{{ $build->version }}"{{ $modpack->latest === $build->version ? " checked" : "" }}></td>
					...
					<td>
						<a href="{{ url('/modpack/build/'.$build->id) }}" class="btn btn-primary btn-xs">Manage</a>
						<a href="{{ url('/modpack/build/'.$build->id, ['action' => 'edit']) }}" class="btn btn-warning btn-xs">Edit</a>
						<a href="{{ url('/modpack/build/'.$build->id, ['action' => 'delete']) }}" class="btn btn-danger btn-xs">Delete</a>
					</td>
				</tr>
			@endforeach
			</tbody>
