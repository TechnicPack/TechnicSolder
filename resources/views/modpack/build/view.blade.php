...
@section('content')
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <div class="col-md-6">
        <label>Java Version: <span
            class="label label-default">{{ !empty($build->min_java) ? $build->min_java : 'Not Required'  }}</span></label><br>
        <label>Memory (<i>in MB</i>): <span
            class="label label-default">{{ $build->min_memory != 0 ? $build->min_memory : 'Not Required' }}</span></label>
      </div>
    </div>
  </div>
  <div class="panel panel-default">
        ...
        <table class="table" id="mod-list">
          ...
          @foreach ($build->modversions->sortByDesc('build_id', SORT_NATURAL) as $ver)
            <tr>
              ...
              <td>
                <form method="post" action="{{ URL::to('modpack/build/modify') }}" style="margin-bottom: 0"
                      class="mod-version">
                  @csrf
                  <input type="hidden" class="build-id" name="build_id" value="{{ $build->id }}">
                  <input type="hidden" class="modversion-id" name="modversion_id"
                  ...
                  <div class="form-group input-group">
                    <select class="form-control" name="version">
                      @foreach ($ver->mod->versions as $version)
                        <option
                          value="{{ $version->id }}"{{ $selected = ($ver->version == $version->version ? 'selected' : '') }}>{{ $version->version }}</option>
                      @endforeach
                    </select>
